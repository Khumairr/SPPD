...
<body onload="window.print()">
    <div class="kop-surat">
        ...
        <p class="nomor-surat">Nomor: {{ $no_spt }}</p>
    </div>
    ...
    <div class="content">
        ...
        <table class="table" style="margin: 15px 0px;">
            <tr>
                ...
                <td rowspan="3" style="vertical-align: top;">:</td>
                <td>{{ $perihal_sppd }}, pada tanggal {{ \Carbon\Carbon::parse($detail->tgl_berangkat)->translatedFormat('d')}} - {{ \Carbon\Carbon::parse($detail->tgl_kembali)->translatedFormat('d F ') }}. <br> Demikian surat perintah tugas ini, untuk dilaksanakan dengan penuh tanggung jawab.</td>
            </tr>
        </table>      

        <div class="footer">
            <div class="tanda-tangan" style="text-align: center;">
                <p>Tangerang Selatan, {{ \Carbon\Carbon::parse($tgl_spt)->translatedFormat('d F Y') }}</p>
                <p><strong>KEPALA,</strong></p>
                <br><br><br>
                <p style="text-decoration: underline;"><strong>Drs, Fuad. MPA</strong></p><!-- {{ $pegawai_satu->nama_staff }} -->
                <p>Pembina Utama Muda</p><!-- {{ $pegawai_satu->jabatan }} -->
                <p>NIP. 19741129 199303 1 003</p><!-- {{ $pegawai_satu->nip }} -->
            </div>
        </div>  
    </div>
</body>
